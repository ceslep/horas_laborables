...
npm run test src/tests/date-validation.test.ts    # Run single test file
...
- **Interfaces/Types**: PascalCase (e.g., `Festivo`, `Teacher`)
...
- Use `any[]` for loosely typed arrays
...
<script lang="ts">
  ...
  let { day, value, onSelect, categories, isCurrentMonth = true, weekday = "" }: Props = $props();
...
### Imports
- Order: Svelte built-ins → External libraries → Internal components/services → Assets
- Use explicit relative imports (e.g., `./CalendarCell.svelte`)
- Group by type with blank lines between groups

### Component Structure
- Place interface Props at the top of `<script>` block
- Use `$state` for reactive variables, `$derived` for computed values, `$effect` for side effects
- Keep template logic minimal; extract to functions

### Error Handling
- Use try/catch with meaningful error messages
- Display errors via SweetAlert2 or custom notification
...
│   ├── services/      # API services (.ts and .svelte.ts)
...
- Don't commit secrets (API keys in environment variables)
- Build output in `dist/` committed for GitHub Pages deployment
